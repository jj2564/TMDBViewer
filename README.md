...
本次會連結到的部分較少，分析出來的關係圖就相當簡略。
![物件關係圖](images/object_relationship.png "Magic Gardens")

`Movie Detail` 為 `Movie` 的繼承 child ，對於 `Favorite` 來說一個收藏可以對應到多個 `Movie` ，那因為其中儲存的為 `id` 因此並不會知道 `Movie`。 本應分為兩張分析圖，但是物件過少就混在一張了。
